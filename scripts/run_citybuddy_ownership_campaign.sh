...
minimum_stateeval_commit="df60ce6f920f83a593057fbf71dc25ab06727755"
...
usage() {
  echo "usage: $0 {preflight|execute|resume} --phase {calibration|formal} --output ABSOLUTE_PATH" >&2
}
...
if (($# == 0)); then
  fail_usage "A command, phase, and output path are required."
fi
...
while (($# > 0)); do
  case "$1" in
    --phase)
      (($# >= 2)) || fail_usage "--phase requires a value."
      [[ -z "$stateeval_phase" ]] || fail_usage "--phase may be specified only once."
      stateeval_phase="$2"
      shift 2
      ;;
    --output)
...
case "$stateeval_phase" in
  calibration)
    stateeval_seed=2026083101
    stateeval_blocks=10
    stateeval_slots=100
    ;;
  formal)
    stateeval_seed=2026083102
    stateeval_blocks=60
    stateeval_slots=600
    ;;
  "") fail_usage "--phase is required." ;;
  *) fail_usage "--phase must be calibration or formal." ;;
esac
[[ -n "$stateeval_output" ]] || fail_usage "--output is required."

# Capture provider configuration before the first child process. Local copies are
# deliberately non-exported and are injected only into the two measured agents.
...
if not isinstance(value, dict):
    raise SystemExit(1)
if value.get("schema") != "stateeval.citybuddy-ownership-campaign/v2":
    raise SystemExit(1)
if value.get("campaign") != "citybuddy-ownership":
    raise SystemExit(1)
if value.get("phase") != sys.argv[2]:
    raise SystemExit(1)
' "$stateeval_output/manifest.json" "$stateeval_phase" >/dev/null 2>&1
}
...
case "$stateeval_command" in
  ...
  resume)
    resume_output_is_valid \
      || fail "resume requires a valid existing campaign manifest for this phase."
    stateeval_output_mode="resume"
    ;;
  preflight)
    if [[ -e "$stateeval_output" ]]; then
      resume_output_is_valid \
        || fail "Existing preflight output is not a resumable campaign for this phase."
      stateeval_output_mode="resume"
    else
      stateeval_output_mode="fresh"
    fi
...
check_stateeval_repository() {
  ...
  git -C "$stateeval_root" merge-base --is-ancestor \
    ...
    || fail "StateEval HEAD does not contain the frozen launcher base."
  git -C "$stateeval_root" cat-file -e \
    "$actual_stateeval_commit:scripts/run_citybuddy_ownership_campaign.sh" 2>/dev/null \
    || fail "StateEval HEAD does not contain the ownership campaign launcher."
...
echo "ownership_campaign_preflight"
...
echo "evaluation_session_propagation_enabled=true"
...
# Everything below this point belongs only to execute/resume. Preflight exits
# before sourcing helpers, allocating runtime state, or touching Docker/network IO.
...
stateeval_project="stateeval-ownership-campaign-$(openssl rand -hex 8)"
stateeval_ownership_off_launch_id="stateeval-ownership-campaign-off-$(openssl rand -hex 12)"
compose=(
  docker compose
...
start_commerce() {
  ...
  SPRING_DATASOURCE_PASSWORD="$stateeval_commerce_app_password" \
    ...
    --citybuddy.evaluation.build-id=stateeval-ownership-campaign \
...
start_agent() {
  ...
  (
    ...
    export \
      ...
      AGENT_EVALUATION_SESSION_PROPAGATION_ENABLED=true \
...
stateeval_mock_payment_key="stateeval-ownership-campaign-$(openssl rand -hex 12)"
...
# Re-check both immutable source boundaries after build and immediately before
# the campaign can create or resume an artifact.
check_repositories
...
stateeval_campaign_args=(
  --phase "$stateeval_phase"
  --output "$stateeval_output"
)
...
(
  ...
  exec python3 -m stateeval.citybuddy_ownership_campaign "${stateeval_campaign_args[@]}"
) &
stateeval_campaign_pid=$!
...
assert_ownership_off_alive || stateeval_continuity_status=$?
